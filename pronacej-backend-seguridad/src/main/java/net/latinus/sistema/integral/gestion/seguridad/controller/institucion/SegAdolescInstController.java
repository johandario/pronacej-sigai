...
import net.latinus.sistema.integral.gestion.seguridad.service.institucion.SeguimientoAdolescInsService;
...
@RestController
@AllArgsConstructor
@RequestMapping(path = "api/v1/seguimiento-adolescente")
@SecurityRequirement(name = "Authorization")
public class SegAdolescInstController {
    private final SeguimientoAdolescInsService seguimientoAdolescInsService;
    ...
    public ResponseEntity<BodyEncriptado> obtenerRegistroSalida(HttpServletRequest httpServletRequest, @RequestBody BodyEncriptado bodyEncriptado) throws Exception {
        ...
        String bodyDesencriptado = bodyEncriptado.desencriptarPorEmpresa(this.parametroDelSistemaRepository, null)
                .getData();
        RespuestaPorDefectoAuditoria<PaginacionResponse<SeguimientoAdolescInstDTO>> df = seguimientoAdolescInsService.obtenerInstituciones(httpServletRequest, bodyEncriptado);
        this.auditoriaAccionesSistemaService.guardarAccionRequestEncriptado(httpServletRequest,
                bodyDesencriptado, df, fechaInicio, EtiquetaNemonico.ACCION_BUSCAR_SEGUIMIENTO_ADOLESCENTE);
    ...
    public ResponseEntity<BodyEncriptado> crearRegistroInstitucion(HttpServletRequest httpServletRequest,
        ...
        RespuestaPorDefectoAuditoria<SeguimientoAdolescInstDTO> respuesta = seguimientoAdolescInsService.crearRegistroInstitucion(httpServletRequest, bodyEncriptado);
        String accion;
        if (respuesta.getData() != null && Boolean.TRUE.equals(respuesta.getData().getEsEdicion())) {
            accion = EtiquetaNemonico.ACCION_ACTUALIZAR_SEGUIMIENTO_ADOLESCENTE;
        } else {
            accion = EtiquetaNemonico.ACCION_CREAR_SEGUIMIENTO_ADOLESCENTE;
        }
    ...
    public ResponseEntity<BodyEncriptado> obtenerRegistroInstitucionPorToken(HttpServletRequest httpServletRequest,
        ...
        RespuestaPorDefectoAuditoria<SeguimientoAdolescInstDTO> df = this.seguimientoAdolescInsService.obtenerRegistroInstitucionPorToken(httpServletRequest, ID);
        return ResponseEntity.ok(df.transFormarEnbodyEncriptado(this.parametroDelSistemaRepository, null));
    }
    ...
    public ResponseEntity<BodyEncriptado> eliminarRegistroInstitucion(HttpServletRequest httpServletRequest,
        ...
        Date fechaInicio = new Date();
        String bodyDesencriptado = bodyEncriptado.desencriptarPorEmpresa(this.parametroDelSistemaRepository, null).getData();
        RespuestaPorDefectoAuditoria<Boolean> df = this.seguimientoAdolescInsService.eliminarRegistroInstitucion(httpServletRequest, bodyEncriptado);
        this.auditoriaAccionesSistemaService.guardarAccionRequestEncriptado(httpServletRequest,
                bodyDesencriptado, df, fechaInicio, EtiquetaNemonico.ACCION_ELIMINAR_SEGUIMIENTO_ADOLESCENTE);
        return ResponseEntity.ok(df.transFormarEnbodyEncriptado(this.parametroDelSistemaRepository, null));
    }

    @GetMapping("/obtenerInstituciones")
    public RespuestaPorDefectoAuditoria<List<SeguimientoAdolescInstDTO>> obtenerTodasLasInstituciones(HttpServletRequest request) {
        return seguimientoAdolescInsService.obtenerTodasLasInstituciones(request);
    }
