...
export class TreeNgxComponent implements OnInit, OnDestroy, OnChanges, TreeNgx {
  ...
  @Output() clickArrowRigthEvent = new EventEmitter<NodeItem<any>>();
  ...
  // clickArrowRigthEvent2(event: NodeState) {
  //   this.clickArrowRigthEvent.emit(event.nodeItem);
  // }

  clickArrowRigthEvent2(event: NodeState) {
  const nodeItem = event?.nodeItem;
  if (nodeItem) {
    this.clickArrowRigthEvent.emit(nodeItem);
  }
}
