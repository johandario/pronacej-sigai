...
export class TreeService {
  ...
  public getChildrenIds(parentId: string): string[] {
  const parentNode = this.findNodeByIdRecursive(this.treeState, parentId);
  if (parentNode && parentNode.children) {
    return parentNode.children.map(child => child.nodeItem.id);
  }
  return [];
}

private findNodeByIdRecursive(nodes: NodeState[], id: string): NodeState | null {
  for (const node of nodes) {
    if (node.nodeItem.id === id) {
      return node;
    }
    if (node.children) {
