...
@Component({
  selector: 'app-modal-subir-docs-segui-pti',
  standalone: true,
  imports: [
    MatDialogTitle,
    MatDialogContent,
  ...
  templateUrl: './modal-subir-docs-segui-pti.component.html',
  styleUrl: './modal-subir-docs-segui-pti.component.scss'
})
export class ModalSubirDocsSeguiPtiComponent implements OnInit {

  tiposDeDocumentosSistema: TipoDeDocumento[];  

  constructor(
    private dialogRef: MatDialogRef<ModalSubirDocsSeguiPtiComponent>,   
  ...
  subirArchivosEvent(documentos: DocumentoSubido[]) {
    if (documentos && documentos.length > 0) {
      ...
      for (let documentoSubido of documentos) {
        let planTratamientoIndSeguiDocumentoDTO = new PlanTratamientoIndSeguiDocumentoDTO();
        planTratamientoIndSeguiDocumentoDTO.tokenIdentificadorSeguimiento = this.data.tokenIdentificador;
        planTratamientoIndSeguiDocumentoDTO.documentoDTO = documentoSubido.documentoDTO;

        let load = this.dialogMensajeService.mensajeLoading("Subiendo el documento: " +
          documentoSubido.documento.name
        );
        this.planTratamientoService.subirDocumentoSeguimiento(
