...
import { ActividadIntervencionDTO, ActividadIntervencionSeguimientoDTO, PlanTratamientoIndDTO, PlanTratamientoIndIntervDTO } from 'app/core/model/both/planTratamientoIndDTO.model';
...
import { ActividadIntervencionDialogComponent } from '../consolidar-intervencion/actividad-intervencion-dialog/actividad-intervencion-dialog.component';
import {Location} from '@angular/common';
import { MatTooltipModule } from '@angular/material/tooltip';
import { ModalSegSubactPtiComponent } from './modal-seg-subact-pti/modal-seg-subact-pti.component';
import { FuncionesUtils } from 'app/core/utils/funcionesUtils.model';
import etiquetasModel from 'app/core/etiquetas.model';

@Component({
  selector: 'app-subactividades-pti',
  standalone: true,
  imports: [
    MatStepperModule,
    FormsModule,
  ...
  templateUrl: './subactividades-pti.component.html',
  styleUrl: './subactividades-pti.component.scss',
  providers: [provideNativeDateAdapter(),]
})
export class SubactividadesPtiComponent {
  mostrarSeguimiento: boolean = false;
  ...
  @ViewChild('paginatorSeguimientos') paginatorSeguimientos: MatPaginator;
  ...
  ejecutarAccion(): void {
    ...
    this.planTratamientoService.actualizarIntervencion(intervencionDTO,this.nemonicoMenu).subscribe({
      next: (
  ...
  obtenerIntervencion(numInterv: string) {
    this.planTratamientoService.obtenerIntervencion(numInterv,this.nemonicoMenu).subscribe(item => {
  ...
  obtenerActividadesIntervencion() {
    ...
    this.planTratamientoService
      .obtenerActividadesTratamiento(paginacionRequest, this.nemonicoMenu)
      .subscribe({
        next: (
          ...
          this.dataSourceActividades.data.map(actividad => actividad.activo = false);
  ...
  mostrarActividadIntervencion(actividad: ActividadIntervencionDTO) {
    const dialogRef = this.dialog.open(ActividadIntervencionDialogComponent, {      
      width: '600px',
      data: { actividad: actividad, frecuencia: this.listaTipoFrecuencia, visualizar: true }
    });
  }
    ...
    ref.afterClosed().subscribe(
      {
        next: (resp: "confirmed" | "cancelled") => {
          if (resp == "confirmed") {
            this.planTratamientoService.eliminarActividadIntervencion(actividad.idActividadIntervencion, this.nemonicoMenu).subscribe({
              next: (response) => {
                if (response.exito) {
                  this.obtenerActividadesIntervencion();                  
                } else {
  ...
  obtenerSeguimientos(actividad: ActividadIntervencionDTO) {
    this.dataSourceActividades.data.map(actividad => actividad.activo = false);
    this.mostrarSeguimiento = true;
    this.actividadInterv = actividad;
    this.actividadInterv.activo = true;
    console.log(this.actividadInterv);
    this.obtenerSeguimientosActividad(actividad);
  }

  obtenerSeguimientosActividad(actividad: ActividadIntervencionDTO) {
    let paginacionRequest = new PaginacionRequest();
    paginacionRequest.size = this.size;
    paginacionRequest.page = this.page;
    paginacionRequest.filter = (actividad.idActividadIntervencion).toString();
    ...
    this.planTratamientoService
      ...
      .subscribe({
        next: (response: RespuestaPorDefecto<PaginacionResponse<ActividadIntervencionSeguimientoDTO>>) => {
          ...
          this.totalItems = response.data.totalItems;
          this.dataSourceSeguimientos.paginator = this.paginatorSeguimientos;
          const element = document.getElementById('seccionSeguimiento');
          if (element) {
  ...
  agregarSeguimiento() {
    const dialogRef = this.dialog.open(ModalSegSubactPtiComponent, {
      disableClose: true,
      width: '600px',      
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        let nuevoSeguimiento: ActividadIntervencionSeguimientoDTO = result;
        nuevoSeguimiento.idActividadIntervencion = this.actividadInterv.idActividadIntervencion;
        this.planTratamientoService.crearActualizarSeguimientoActividad(result, this.nemonicoMenu).subscribe({
          next: response => {            
            this.obtenerSeguimientosActividad(nuevoSeguimiento);
          },
          error: error => {
            this.planTratamientoService.checkError(error);
            console.error(error);
  ...
  visualizarSeguimiento(seguimiento: ActividadIntervencionSeguimientoDTO, index: number) {
    const dialogRef = this.dialog.open(ModalSegSubactPtiComponent, {     
      width: '600px',   
      data: {fila: seguimiento, visualizar: true}  
    });
    
  }

  editarSeguimiento(seguimiento: ActividadIntervencionSeguimientoDTO, index: number) {
    const dialogRef = this.dialog.open(ModalSegSubactPtiComponent, {
      disableClose: true,
      width: '600px',   
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        let nuevoSeguimiento: ActividadIntervencionSeguimientoDTO = result;
        nuevoSeguimiento.idActividadIntervencionSeguimiento = seguimiento.idActividadIntervencionSeguimiento;
        nuevoSeguimiento.idActividadIntervencion = this.actividadInterv.idActividadIntervencion;
        nuevoSeguimiento.esEdicion = true;
        this.planTratamientoService.crearActualizarSeguimientoActividad(result, this.nemonicoMenu).subscribe({
          next: response => {            
            this.obtenerSeguimientosActividad(nuevoSeguimiento);
          },
